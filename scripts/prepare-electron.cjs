...
/* 4. 生成模板数据库（含 schema + 内置种子数据），随包发布供首次启动复制
 *     必须在 electron-rebuild 之前执行，因为 seed.ts 使用系统 Node.js 的 ABI，
 *     而 electron-rebuild 会将 better-sqlite3 重编译为 Electron ABI */
...
/* 5. @electron/rebuild：将 better-sqlite3 重编为 Electron ABI（Windows 带 win_delay_load_hook） */
...
/* 6. 用 Electron-ABI 的 .node 覆盖 standalone 内追踪进来的 Node-ABI 版本 */
